...
Same requirments apply to cryptocurrency like Bitcoin as well. If it fails either of them, it loses the trust. In this post, I'm building a simplified version (yes, no fancy Merkle tree, script, proof of work, etc.) of Bitcoin transaction verification system - with the end goal to fool it, and lose trust on it.

## What does a Bitcoin transaction look like ? 

A [Bitcoin transaction](https://en.bitcoin.it/wiki/Transaction) works like a [double-entry bookkeeping ledger](https://en.wikipedia.org/wiki/Double-entry_bookkeeping), which contains all the information about a money movement. Most standard P2P transactions consist of `inputs` and `outputs`, where the former describes where the fund comes from and the latter describes where it’ll end up at. [Coinbase transaction](https://bitcoin.org/en/glossary/coinbase-transaction) is an exception that doesn’t have any `inputs` because instead of transferred from a previous owner, it’s created by miners. Also, each transaction can be identified with a unique [Transaction Identifier (TXID)](https://bitcoin.org/en/glossary/txid)`, which is the hash of raw transaction data.
...
Based on these principles, we can try building that simplified version of Bitcoin transaction verification system. My implementation can be found [here](https://gist.github.com/yvetterowe/de42b30a7ef5cdc6e5c5a0312302c3f3).


## Let’s break things.

Things only go well when it go well. Assume Alice 👩🏻‍🌾 and Bob 🦹🏻‍♂️ are usually good citizens, but sometimes they can be bad actors as well. All toy examples below can be found and run [here](https://gist.github.com/yvetterowe/de42b30a7ef5cdc6e5c5a0312302c3f3).
...
#### Case 5 - 👩🏻‍🌾 tries paying 💰💰 to 🦹🏻‍♂️ by faking previous transaction amount
She needs to pay 🦹🏻‍♂️ 💰💰, but she only owns 💰. She’s gonna try her luck by pretending she owns 💰💰.
...
Hmm this is weird because she DID sign with her own signature. It turns out that even if she tries claiming previous transactions indeed payable to her, she still can’t pass the signature verification because transaction amount is modified.
...
She’s just really gritty, I know. This time, she tries another strategy by claiming same previous transaction payable to her multiple times - so that she doesn't need to modify the transaction amount as before. 💰x 2 = 💰💰,  what a beautiful math.
{% splash %}
